...
> **Note**: If you access IPs in `FIREWALL_OUTBOUND_SUBNETS` through a DNS entry that points to them, they need to be in `DNS_REBINDING_PROTECTION_EXEMPT_HOSTNAMES` as gluetun blocks dns entries pointing to private IP addresses by default (DNS rebinding protection).
