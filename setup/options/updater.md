...
| `UPDATER_PERIOD` | `0` | Valid duration string such as `480h` | Period to update the VPN servers data in memory and to /gluetun/servers.json. Set to `0` to disable. This does a burst of DNS over HTTPs requests, which may be blocked if you set `BLOCK_MALICIOUS=on` for example. |
