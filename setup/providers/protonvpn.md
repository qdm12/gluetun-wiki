...
- `UPDATER_PROTONVPN_TOTP_SECRET`: If your account uses 2FA TOTP, you can set this option to your TOTP hex secret key. This one can be obtained through the Proton web ui when turning on 2FA TOTP. Gluetun will then generate codes for every servers data update. Alternatively, you can use [update using the command line](https://github.com/qdm12/gluetun-wiki/blob/main/setup/servers.md#update-using-the-command-line) and use `-proton-totp-code=123456` to provide a TOTP code generated by your authenticator app, to limit exposing your TOTP secret key.
